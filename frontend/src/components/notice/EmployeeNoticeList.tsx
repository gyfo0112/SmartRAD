  ...
  return (
    <div className="space-y-3">
      {!canManage && pinnedNotice && (
        <section className="flex flex-col items-stretch justify-between gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 sm:flex-row sm:items-center sm:gap-4" aria-label="상단 고정 공지">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded bg-amber-100 text-sm">📌</span>
            <div className="min-w-0"><p className="text-sm font-bold text-amber-800">상단 고정 공지</p><p className="line-clamp-2 text-base font-semibold text-amber-950">{pinnedNotice.title} <span className="ml-1 rounded bg-rose-500 px-1.5 py-0.5 text-[10px] text-white">NEW</span></p></div>
          </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
        <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-[80px_132px_minmax(220px,1fr)_auto] md:items-end">
          ...
          <label className="text-sm font-semibold text-slate-600 sm:col-span-2 md:col-span-1">제목 검색<div className="relative mt-1"><MagnifyingGlassIcon className="pointer-events-none absolute left-2 top-2 h-4 w-4 text-slate-400" /><input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") runSearch(); }} placeholder="제목으로 검색" className="h-9 w-full rounded-md border border-slate-200 pl-7 pr-3 text-sm focus:border-indigo-500 focus:outline-none" /></div></label>
          <div className="flex gap-1 sm:col-span-2 md:col-span-1"><button type="button" onClick={resetFilters} className="inline-flex h-9 flex-1 items-center justify-center gap-1 rounded-md border border-slate-200 px-3 text-sm font-medium text-slate-600 hover:bg-slate-50 md:mt-5 md:flex-none"><ArrowPathIcon className="h-3.5 w-3.5" />초기화</button><button type="button" onClick={runSearch} className="inline-flex h-9 flex-1 items-center justify-center gap-1 rounded-md bg-indigo-600 px-4 text-sm font-bold text-white hover:bg-indigo-700 md:mt-5 md:flex-none"><MagnifyingGlassIcon className="h-3.5 w-3.5" />조회</button></div>
        </div>
        {(startDate || endDate) && <p className="mt-2 text-xs font-medium text-indigo-600">적용 기간: {startDate || "전체"} ~ {endDate || "전체"}</p>}
      </section>
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div className="divide-y divide-slate-100 sm:hidden">
          {loading ? (
            ...
            <article key={row.noticeId} className={row.pinned ? "space-y-2 bg-amber-50/70 px-4 py-4" : "space-y-2 px-4 py-4"}>
              <div className="flex items-start justify-between gap-3">
                <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-bold ${row.pinned ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{noticeType(row)}</span>
                <span className="text-xs text-slate-400">{formatDate(row.createdAt)}</span>
              </div>
              <button type="button" onClick={() => openView(row.noticeId)} className="block w-full text-left text-sm font-semibold text-slate-800 hover:text-indigo-600">
                {row.pinned && <span className="mr-1 text-amber-500">📌</span>}
                {row.title}
                {row.pinned && <span className="ml-2 rounded bg-rose-500 px-1 py-0.5 text-[10px] text-white">NEW</span>}
              </button>
              <div className="flex items-center justify-between text-xs text-slate-500">
                <span>{row.writerName} · 조회 {row.viewCount}</span>
                <button type="button" onClick={() => openView(row.noticeId)} className="rounded border border-slate-200 bg-slate-50 px-2 py-1 font-semibold text-slate-600">상세보기</button>
              </div>
            </article>
          ))}
        </div>
        <div className="employee-notice-pagination flex items-center justify-between border-t border-slate-100 px-4 py-2"><p className="text-xs text-slate-400">총 {totalElements}건{canManage && ` · ${checkedIds.length}건 선택`}</p><div className="flex gap-1 mr-20"><button type="button" disabled={page === 0} onClick={() => setPage((value) => value - 1)} className="rounded border border-slate-200 p-1 text-slate-400 disabled:opacity-40"><ChevronLeftIcon className="h-3.5 w-3.5" /></button><span className="rounded bg-indigo-600 px-2 py-1 text-xs font-bold text-white">{page + 1}</span><button type="button" disabled={page >= totalPages - 1} onClick={() => setPage((value) => value + 1)} className="rounded border border-slate-200 p-1 text-slate-400 disabled:opacity-40"><ChevronRightIcon className="h-3.5 w-3.5" /></button></div></div>
      </section>
