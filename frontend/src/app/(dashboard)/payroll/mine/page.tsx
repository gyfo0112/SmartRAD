    ...
    <div className="payroll-statement-print mx-auto max-w-[1600px] space-y-4 text-slate-800">
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <div><h3 className="font-bold text-slate-900">급여 명세서 목록</h3><p className="mt-0.5 text-xs text-slate-400">지급월을 선택하면 상세 내역을 확인할 수 있습니다.</p></div>
          <span className="rounded-md border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-slate-600">총 {payrolls.length}건</span>
        </div>
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div className="flex items-center gap-2"><DocumentTextIcon className="h-5 w-5 text-indigo-500" /><div><h3 className="font-bold text-slate-900">이번달 명세서 요약</h3><p className="text-xs text-slate-400">{selectedPayroll ? `${formatMonth(selectedPayroll.payrollYearMonth)} 급여 내역` : "급여 내역을 선택해주세요."}</p></div></div>{selectedPayroll && <span className="rounded-full border border-indigo-100 bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-600">{formatMonth(selectedPayroll.payrollYearMonth)} · {statusLabel(selectedPayroll.payrollStatusCode)}</span>}</div>
        {detailLoading ? <div className="px-5 py-12 text-center text-sm text-slate-400">상세 내역을 불러오는 중입니다.</div> : detail ? <div className="grid gap-0 lg:grid-cols-[1fr_1fr_1.05fr]"><StatementItems title="지급 내역" items={earnings} total={totalEarnings} totalLabel="지급 총액" /><StatementItems title="공제 내역" items={deductions} total={totalDeductions} totalLabel="공제 합계" negative /><div className="border-t border-slate-100 p-5 lg:border-l lg:border-t-0"><p className="text-sm font-semibold text-slate-700">최종 정산</p><div className="mt-4 space-y-2 text-sm"><div className="flex justify-between text-slate-500"><span>세전 총액</span><strong className="text-slate-800">{formatCurrency(totalEarnings)}</strong></div><div className="flex justify-between text-slate-500"><span>공제 합계</span><strong className="text-rose-500">-{formatCurrency(totalDeductions)}</strong></div></div><div className="mt-4 rounded-lg bg-gradient-to-r from-indigo-600 to-violet-500 p-4 text-white"><p className="text-xs font-medium text-indigo-100">실수령액</p><p className="mt-1 text-2xl font-bold">{formatCurrency(netPay)}</p></div><div className="mt-3 grid gap-2"><button onClick={() => window.print()} className="inline-flex items-center justify-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-100"><PrinterIcon className="h-4 w-4" /> 명세서 인쇄</button><button onClick={downloadStatement} className="inline-flex items-center justify-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 hover:bg-emerald-100"><DocumentArrowDownIcon className="h-4 w-4" /> 파일 저장</button></div></div></div> : <div className="px-5 py-12 text-center text-sm text-slate-400">상세 내역을 선택해주세요.</div>}
      </section>
    </div>
  );
}
...
function SummaryCard({ label, value, caption, icon: Icon, tone }: { label: string; value: string; caption: string; icon: typeof WalletIcon; tone: "indigo" | "emerald" | "rose" | "violet" }) {
  ...
  return <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between"><div><p className="text-xs font-medium text-slate-500">{label}</p><p className="mt-1 text-xl font-bold tracking-tight text-slate-900">{value}</p><p className="mt-1 text-[11px] text-slate-400">{caption}</p></div><span className={`rounded-full p-3 ${colors[tone]}`}><Icon className="h-5 w-5" /></span></div></div>;
}
...
function StatementItems({ title, items, total, totalLabel, negative = false }: { title: string; items: PayrollDetail[]; total: number; totalLabel: string; negative?: boolean }) {
  return <div className="border-b border-slate-100 p-5 lg:border-b-0 lg:border-r"><p className="text-sm font-semibold text-slate-700">{title}</p><div className="mt-4 space-y-2 text-xs">{items.length ? items.map((item) => <div key={`${item.itemName}-${item.amount}`} className="flex justify-between"><span className="text-slate-500">{item.itemName}</span><strong className={negative ? "text-rose-500" : "text-slate-800"}>{negative && "-"}{formatCurrency(item.amount)}</strong></div>) : <p className="text-slate-400">등록된 항목이 없습니다.</p>}</div><div className={`mt-4 flex justify-between rounded-md px-3 py-2 text-xs font-bold ${negative ? "bg-rose-50 text-rose-600" : "bg-slate-100 text-slate-700"}`}><span>{totalLabel}</span><span>{negative && "-"}{formatCurrency(total)}</span></div></div>;
}